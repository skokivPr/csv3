...
*/
/* Vehicle Data Command Center - enhanced-spreadsheet.js
 * Version: 3.0.0 (Custom Table Component)
 * Author: skoki
...
// Application Notification System (separate from console)
const notificationSystem = {
    ...
    init() {
        ...
        this.fullContainer = document.getElementById('notificationsFullOutput');
        ...
        const expandBtn = document.getElementById('expandNotificationsBtn');
        if (expandBtn) {
            expandBtn.addEventListener('click', () => this.showFullView());
        }
        ...
        const modalOverlay = document.getElementById('notificationsModalOverlay');
        if (modalOverlay) {
            modalOverlay.addEventListener('click', (e) => {
                if (e.target === modalOverlay) this.closeFullView();
            });

            const closeBtn = modalOverlay.querySelector('.close-btn');
            if (closeBtn) {
                closeBtn.addEventListener('click', () => this.closeFullView());
            }
        }
    ...
    show(message, type = 'info', duration = 3000) {
        ...
        console[consoleMethod](`${emoji} [${timestamp}] ${message}`);
    ...
    addMessage(message, type = 'info') {
        ...
        // Update compact view (show only last 3 messages)
        this.updateCompactView();

        // Update full view if modal is open
        this.updateFullView();
    },
    ...
    updateFullView() {
        ...
        this.allMessages.forEach(msg => {
            ...
            this.fullContainer.appendChild(notificationEntry);
        });
    ...
    clear() {
        // Clear only recent messages (keep full history)
        this.updateCompactView();
    },
...
const jsConsole = {
    ...
    init() {
        ...
        // Welcome message
        this.addEntry('JavaScript Console ready! Type code and press Enter.', 'info');
        this.addEntry('Type help() for available commands and examples.', 'info');

        // Add event listeners
        this.input.addEventListener('keydown', (e) => this.handleKeyDown(e));
        this.input.addEventListener('input', (e) => this.handleInput(e));

        // Add clear console button listener
    ...
    getGlobalSuggestions() {
        return [
            'state', 'tableManager', 'utils', 'customModal', 'notificationSystem',
            'console', 'Math', 'Date', 'JSON', 'Array', 'Object', 'String', 'Number', 'Boolean',
            'help', 'clear', 'typeof', 'instanceof', 'new', 'function', 'var', 'let', 'const',
    ...
    getPropertySuggestions(objectPath, property) {
        try {
            const obj = this.safeEval(objectPath);
            if (obj && typeof obj === 'object') {
                const props = Object.getOwnPropertyNames(obj)
                    .concat(Object.getOwnPropertyNames(Object.getPrototypeOf(obj) || {}))
                    .filter(prop => prop.toLowerCase().startsWith(property.toLowerCase()))
                    .slice(0, 10); // Limit suggestions
                return props;
            }
        } catch (e) {
            // Ignore errors during suggestion generation
        }
        return [];
    },
    ...
    executeCommand() {
        ...
        if (this.history[this.history.length - 1] !== command) {
            ...
            if (this.history.length > 50) {
                this.history.shift(); // Keep only last 50 commands
            }
        }
        this.historyIndex = -1;
        ...
        // Execute command
        try {
            let result = this.safeEval(command);
            this.addResultEntry(result, 'result');
        } catch (error) {
            this.addResultEntry(error.message, 'error');
        }
    },
    ...
    safeEval(code) {
        ...
        const context = {
            ...
            console: {
                log: (...args) => this.addResultEntry(args.join(' '), 'result'),
                error: (...args) => this.addResultEntry(args.join(' '), 'error'),
                warn: (...args) => this.addResultEntry(args.join(' '), 'warning'),
                info: (...args) => this.addResultEntry(args.join(' '), 'info')
            },
            ...
            // Utility functions
            help: () => {
                return `JavaScript Console Help:

GLOBALS:
- state: Application state (unifiedData, currentViewData, etc.)
- tableManager: Table operations (render, setSort, etc.)
- utils: Utility functions (validateData, exportToFile, etc.)
- customModal: Modal system (show modals)
...
- JSON.stringify(state.sort)`;
    ...
    addCommandEntry(command) {
        ...
        entry.className = 'console-entry command';
        entry.innerHTML = `<span style="color: var(--highlight-color);">></span> ${this.escapeHtml(command)}`;
        this.container.appendChild(entry);
        this.scrollToBottom();
    },

    addResultEntry(result, type = 'result') {
        const entry = document.createElement('div');
        entry.className = `console-entry result ${type}`;
        ...
        if (result === undefined) {
            ...
            displayValue = `"${this.escapeHtml(result)}"`;
            resultClass = 'string';
        ...
        entry.innerHTML = `<pre>${displayValue}</pre>`;
        this.container.appendChild(entry);
        this.scrollToBottom();
    },
    ...
    addEntry(message, type = 'info') {
        ...
        entry.innerHTML = `<div class="console-message">${this.escapeHtml(message)}</div>`;
        this.container.appendChild(entry);
        this.scrollToBottom();
    },
...
const eventHandlers = {
    ...
    handleCopyToClipboard: () => {
        ...
        navigator.clipboard.writeText(csvString).then(() => {
            showSuccessMessage("Formatted data copied to clipboard!");
        }).catch(err => {
            console.error('Failed to copy text: ', err);
            showErrorMessage("Could not copy data to clipboard.");
        });
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    // Initialize custom modal, console, notification and status systems
    customModal.init();
    jsConsole.init();
